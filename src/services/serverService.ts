...
export class ServerService {
	...
	private async loadAll(): Promise<void> {
		await Promise.all([this.loadServers(), this.loadGroups()])
	}
	...
	private async loadServers(): Promise<void> {
		...
		if (serversJson) {
			try {
				...
				console.log(
					this.localization.localize('service.loadSuccess', this.servers.length)
				)
			} catch (error) {
	...
	private async loadGroups(): Promise<void> {
		...
		console.log(
			'Загрузка групп:',
			groupsJson ? 'найдены группы' : 'группы не найдены'
		)
	...
	private async saveServers(): Promise<void> {
		...
		console.log(
			this.localization.localize('service.saveStart', this.servers.length)
		)
	...
	public async getServers(): Promise<Server[]> {
		...
		console.log(
			this.localization.localize('service.getServers', this.servers.length)
		)
		return [...this.servers]
	}
	...
	public async updateServer(server: Server): Promise<void> {
		...
		if (index !== -1) {
			...
			console.error(
				this.localization.localize('service.updateNotFound', server.id)
			)
			throw new Error(
				this.localization.localize('service.updateNotFound', server.id)
			)
	...
	public async moveServerToGroup(
		serverId: string,
		groupId?: string
	): Promise<void> {
		console.log(
			`Перемещение сервера ${serverId} в группу ${groupId || 'без группы'}`
		)
