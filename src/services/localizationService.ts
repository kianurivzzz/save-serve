...
export class LocalizationService {
	...
	private loadLanguages(): void {
		try {
			...
			console.log(
				`Количество ключей для ru: ${Object.keys(this.messages['ru'] || {}).length}`
			)
			console.log(
				`Количество ключей для en: ${Object.keys(this.messages['en'] || {}).length}`
			)
	...
	private formatString(message: string, args: any[]): string {
		...
		for (let i = 0; i < args.length; i++) {
			...
			result = result.replace(
				placeholder,
				args[i] !== undefined ? args[i].toString() : ''
			)
		}
		return result
	}
