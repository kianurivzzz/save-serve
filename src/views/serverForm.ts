...
export class ServerForm {
	...
	async showAddServerForm(): Promise<Server | undefined> {
		try {
			...
			if (serverData) {
				console.log(
					this.localization.localize('log.serverDataCollected') + ':',
	...
	async showEditServerForm(serverToEdit: Server): Promise<Server | undefined> {
		try {
			console.log(
				this.localization.localize('log.openEditForm', serverToEdit.id)
			)
			const serverData = await this.collectServerInfo(serverToEdit)
			if (serverData) {
				console.log(
					this.localization.localize('log.serverDataUpdated') + ':',
					JSON.stringify(serverData, null, 2)
				)
	...
	private async collectServerInfo(
		existingServer?: Server
	): Promise<Omit<Server, 'id'> | undefined> {
		...
		const name = await vscode.window.showInputBox({
			...
			validateInput: (value: string) =>
				value ? null : this.localization.localize('form.nameValidation')
		})
		if (name === undefined) {
		...
		const host = await vscode.window.showInputBox({
			...
			validateInput: (value: string) =>
				value ? null : this.localization.localize('form.hostValidation')
		})
		if (host === undefined) {
		...
		const portStr = await vscode.window.showInputBox({
			...
			validateInput: (value: string) => {
				...
				return !isNaN(port) && port > 0 && port < 65536
					? null
					: this.localization.localize('form.portValidation')
			}
		})
		...
		const username = await vscode.window.showInputBox({
			...
			validateInput: (value: string) =>
				value ? null : this.localization.localize('form.usernameValidation')
		})
		...
		if (groups.length > 0) {
			...
			const groupOptions: GroupOption[] = [
				{
					label: this.localization.localize('form.noGroup') || 'Без группы',
					id: undefined
				}
			]
			...
			groupOptions.push({
				label:
					this.localization.localize('form.createGroup') ||
			...
			const selectedGroup = await vscode.window.showQuickPick(groupOptions, {
				placeHolder:
					this.localization.localize('form.selectGroup') ||
					'Выберите группу для сервера'
			})
			...
			if (selectedGroup.id === 'create') {
				const newGroupName = await vscode.window.showInputBox({
					prompt:
						this.localization.localize('form.newGroupName') ||
						'Введите название новой группы',
					validateInput: (value: string) =>
						value
							? null
							: this.localization.localize('form.groupNameValidation') ||
				...
				const groupDescription = await vscode.window.showInputBox({
					prompt:
						this.localization.localize('form.groupDescription') ||
				...
				console.log(
					`Выбрана группа: ${selectedGroup.label} (${groupId || 'без группы'})`
				)
			}
		}
		...
		const usePrivateKey =
			authMethod === this.localization.localize('form.authSshKey')
		...
		if (usePrivateKey) {
			...
			console.log(
				this.localization.localize('log.sshKeySelection', privateKeyPath)
			)
