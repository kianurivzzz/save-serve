...
export class ServerTreeProvider implements vscode.TreeDataProvider<TreeItem> {
	...
	async getChildren(element?: TreeItem): Promise<TreeItem[]> {
		if (!element) {
			...
			const groupItems: TreeItem[] = groups.map(
				(group) => new GroupTreeItem(group, this.localization)
			)
			...
			const servers = await this.serverService.getServersInGroup(
				element.group.id
			)
			return servers.map(
				(server) => new ServerTreeItem(server, this.localization)
			)
...
export class UngroupedServersItem extends TreeItem {
	constructor(private localization: LocalizationService) {
		super(
			localization.localize('group.ungrouped') || 'Без группы',
...
export class ServerTreeItem extends TreeItem {
	constructor(
		...
		this.iconPath = new vscode.ThemeIcon(
			iconId,
			server.color ? new vscode.ThemeColor(server.color) : undefined
		)
