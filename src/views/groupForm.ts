...
export class GroupForm {
	...
	async showEditGroupForm(
		groupToEdit: ServerGroup
	): Promise<ServerGroup | undefined> {
		try {
			console.log(
				this.localization.localize('log.editGroupCommand', groupToEdit.id)
			)
	...
	private async collectGroupInfo(
		existingGroup?: ServerGroup
	): Promise<Omit<ServerGroup, 'id'> | undefined> {
		// Имя группы
		const name = await vscode.window.showInputBox({
			prompt: this.localization.localize('form.newGroupName'),
			value: existingGroup?.name || '',
			validateInput: (value: string) =>
				value ? null : this.localization.localize('form.groupNameValidation')
		})
		if (name === undefined) {
			return undefined
		}
